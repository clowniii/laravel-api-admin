...
use function app\util\mb_convert_encoding;
...
class Strs {
    ...
    public static function buildCountRand(int $number, int $length = 4, int $mode = 1): array {
        if ($mode == 1 && $length < strlen((string)$number)) {
            //不足以生成一定数量的不重复数字
            return [];
        }
        $rand = [];
    ...
    // 自动转换字符集 支持数组转换
    public static function autoCharset(string $string, string $from = 'gbk', string $to = 'utf-8')
    {
